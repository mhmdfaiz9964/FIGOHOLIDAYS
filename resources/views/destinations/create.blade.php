<x-app-layout>
    <div x-data="destinationEditor()" class="space-y-10 animate-in fade-in duration-700 pb-20">
        ...
        <div class="flex items-center gap-6">
            ...
            <div>
                <h1 class="text-4xl font-black text-slate-900 tracking-tight">Add Destination</h1>
                <p class="text-slate-400 mt-2 font-semibold">Define locations, provinces, and key attractions.</p>
            </div>
        </div>

        <form action="{{ route('destinations.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            @csrf
                ...
                <div class="bg-white rounded-[2.5rem] border border-slate-50 shadow-sm p-10 space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-2 lg:col-span-2">
                            ...
                            <input type="text" name="name" required class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-900 focus:bg-white focus:border-[#0F4A3B]/20 outline-none transition-all" placeholder="e.g. Kandy, Sri Lanka">
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="label" class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-900 focus:bg-white focus:border-[#0F4A3B]/20 outline-none transition-all" placeholder="e.g. Popular, Historic">
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <template x-for="(attraction, index) in attractions" :key="index">
                                <div class="flex gap-2 p-2 bg-slate-50 rounded-2xl animate-in slide-in-from-top-2">
                                    <input type="text" name="attractions[]" x-model="attractions[index]" class="flex-1 px-4 py-2 bg-white rounded-xl text-sm font-bold outline-none" placeholder="e.g. Temple of the Tooth">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="relative aspect-[4/3] bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200 overflow-hidden group">
                            ...
                            <div x-show="!preview" class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none text-slate-300">
                                ...
                                <p class="text-[10px] font-black uppercase mt-2">Upload Destination</p>
                            </div>
                        </div>
                    </div>
