<x-app-layout>
    <div x-data="hotelEditor()" class="space-y-10 animate-in fade-in duration-700 pb-20">
        ...
        <div class="flex items-center gap-6">
            ...
            <div>
                <h1 class="text-4xl font-black text-slate-900 tracking-tight">Add Hotel</h1>
                <p class="text-slate-400 mt-2 font-semibold">List a new luxury stay or vacation rental.</p>
            </div>
        </div>

        <form action="{{ route('hotels.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            @csrf
                ...
                <div class="bg-white rounded-[2.5rem] border border-slate-50 shadow-sm p-10 space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-2 lg:col-span-2">
                            ...
                            <input type="text" name="name" required value="{{ old('name') }}" class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-900 focus:bg-white focus:border-[#0F4A3B]/20 outline-none transition-all" placeholder="e.g. Grand Paradise Hotel">
                        </div>

                        <div class="space-y-2">
                            <label class="text-xs font-black text-slate-400 uppercase tracking-widest px-1">Location Address</label>
                            <input type="text" name="location" required value="{{ old('location') }}" class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-900 focus:bg-white focus:border-[#0F4A3B]/20 outline-none transition-all" placeholder="Street, Area...">
                        </div>
                                ...
                                <div class="relative flex-1">
                                    <select id="type_select" name="hotel_type_id" required class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-900 focus:bg-white focus:border-[#0F4A3B]/20 outline-none transition-all appearance-none">
                                        ...
                                        @foreach($hotelTypes as $type)
                                            <option value="{{ $type->id }}" {{ old('hotel_type_id') == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" step="0.01" name="price_per_night" required value="{{ old('price_per_night') }}" class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-900 focus:bg-white focus:border-[#0F4A3B]/20 outline-none transition-all" placeholder="0.00">
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="meta_description" rows="3" class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-900 focus:bg-white focus:border-[#0F4A3B]/20 outline-none transition-all" placeholder="SEO description...">{{ old('meta_description') }}</textarea>
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <template x-for="(activity, index) in activities" :key="index">
                                <div class="flex gap-2 p-2 bg-slate-50 rounded-2xl animate-in slide-in-from-top-2">
                                    <input type="text" name="activities[]" x-model="activities[index]" class="flex-1 px-4 py-2 bg-white rounded-xl text-sm font-bold outline-none" placeholder="e.g. Swimming Pool">
