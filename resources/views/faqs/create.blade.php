<x-app-layout>
    <div class="space-y-10 animate-in fade-in duration-700 pb-20">
        ...
        <div class="flex items-center gap-6">
            ...
            <div>
                <h1 class="text-4xl font-black text-slate-900 tracking-tight">Create FAQ</h1>
                <p class="text-slate-400 mt-2 font-semibold">Add a new help topic or policy explanation.</p>
            </div>
        </div>

        <form action="{{ route('faqs.store') }}" method="POST" class="max-w-4xl bg-white rounded-[2.5rem] border border-slate-50 shadow-sm p-10 space-y-8">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                <div class="md:col-span-3 space-y-2">
                    <label class="text-xs font-black text-slate-400 uppercase tracking-widest px-1">Question / Topic</label>
                    <input type="text" name="question" required class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-900 focus:bg-white focus:border-[#0F4A3B]/20 outline-none transition-all" placeholder="e.g. How do I cancel my booking?">
                </div>
                <div class="space-y-2">
                    <label class="text-xs font-black text-slate-400 uppercase tracking-widest px-1">Display Order</label>
                    <input type="number" name="order" value="0" class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-900 focus:bg-white focus:border-[#0F4A3B]/20 outline-none transition-all">
                </div>
            </div>
