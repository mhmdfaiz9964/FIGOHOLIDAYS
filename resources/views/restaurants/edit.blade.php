<x-app-layout>
    <div x-data="restaurantEditor()" class="space-y-10 animate-in fade-in duration-700 pb-20">
        ...
        <div class="flex items-center gap-6">
            ...
            <div>
                <h1 class="text-4xl font-black text-slate-900 tracking-tight">Edit Restaurant</h1>
                <p class="text-slate-400 mt-2 font-semibold">Update restaurant information and availability.</p>
            </div>
        </div>

        <form action="{{ route('restaurants.update', $restaurant) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            @csrf
            @method('PUT')
                ...
                <div class="bg-white rounded-[2.5rem] border border-slate-50 shadow-sm p-10 space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-2 lg:col-span-2">
                            ...
                            <input type="text" name="name" value="{{ old('name', $restaurant->name) }}" required class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-900 focus:bg-white focus:border-[#0F4A3B]/20 outline-none transition-all @error('name') border-rose-500 @enderror">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="type" value="{{ old('type', $restaurant->type) }}" required class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-900 focus:bg-white focus:border-[#0F4A3B]/20 outline-none transition-all @error('type') border-rose-500 @enderror">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="rating" required class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-900 focus:bg-white focus:border-[#0F4A3B]/20 outline-none transition-all appearance-none">
                                @foreach([5,4,3,2,1] as $star)
                                    <option value="{{ $star }}" {{ old('rating', $restaurant->rating) == $star ? 'selected' : '' }}>{{ $star }} Stars</option>
                                @endforeach
                            </select>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="url" name="map_url" value="{{ old('map_url', $restaurant->map_url) }}" class="w-full px-6 py-4 bg-slate-50 border-transparent rounded-2xl font-bold text-slate-900 focus:bg-white focus:border-[#0F4A3B]/20 outline-none transition-all @error('map_url') border-rose-500 @enderror">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="ck-editor-container">
                            <textarea id="editor" name="description">{{ old('description', $restaurant->description) }}</textarea>
                        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="relative aspect-video bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200 overflow-hidden group">
                            ...
                            <template x-if="!preview">
                                ...
                                @else
                                    <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none text-slate-300">
